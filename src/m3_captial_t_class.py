...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        h_rectupperleft = rg.Point(intersection_center.x-(width/2),intersection_center.y-(letter_thickness/2))
        h_rectbottomright = rg.Point(intersection_center.x+(width/2),intersection_center.y+(letter_thickness/2))
        self.h_rect = rg.Rectangle(h_rectupperleft,h_rectbottomright)

        v_rectupperleft = rg.Point(intersection_center.x-(letter_thickness/2),intersection_center.y-(letter_thickness/2))
        v_rectbottomright = rg.Point(intersection_center.x+(letter_thickness/2),intersection_center.y+(height-(letter_thickness/2)))
        self.v_rect = rg.Rectangle(v_rectupperleft,v_rectbottomright)
    ...
    def clone(self):
        ...
        line_thickness = self.h_rect.corner_2.y-self.h_rect.corner_1.y
        width = self.h_rect.corner_2.x-self.h_rect.corner_1.x
        height = self.v_rect.corner_2.y-self.v_rect.corner_1.y
        x = (self.h_rect.corner_2.x-self.h_rect.corner_1.x)/2 + self.h_rect.corner_1.x
        y = (self.h_rect.corner_2.y-self.h_rect.corner_1.y)/2 + self.h_rect.corner_1.y
        intersection_center = rg.Point(x,y)


        T_clone = CapitalT(intersection_center,width,height,line_thickness)
        T_clone.set_colors(self.v_rect.fill_color,self.v_rect.outline_color)
        return T_clone
